...
All chamfer artifacts live under `.chamfer/` in the user's project. The underscore prefix signals tooling-managed — don't edit manually.
...
.chamfer/
  ...
  _generated/      ← generated token files (written by `chamfer build`)
  versions/        ← save history snapshots (written by studio on every save)
```

The studio package itself uses `packages/chamfer-studio/.chamfer/tokens.json` as its dev fixture. Running `yarn dev` from that package finds it automatically via `findChamferTokensFile`.

---

## Dependency Notes

### Zod — do not upgrade chamfer-core to v4 yet

`fetchify` and `http-errors` are already on Zod v4. `chamfer-core` and `chamfer-studio` are on v3.25.x and should stay there for now. The core package has 10 schema files with deep v3 usage (`z.infer`, `safeParse`, discriminated unions, `.default()`) and `zod-to-json-schema` 3.24.x is built specifically for v3. Upgrading for no feature gain with real migration risk isn't worth it until the v4 ecosystem (especially `zod-to-json-schema`) is fully settled. Track as a standalone future upgrade.

### Linaria — required, not optional

The studio's own UI is styled using `@chamfer-css/studio-tokens` — the design token package built specifically for the studio itself. Components call the make functions from that package (e.g. `makeColor`, `makeSpace`) to produce CSS-in-JS class names, which is exactly the same pattern end-users get from their own token builds. Linaria (`@wyw-in-js/vite`) is the extraction layer that turns those make function calls into static CSS at build time.

This means Linaria is load-bearing: removing it would require rewriting all studio component styles to a different CSS-in-JS mechanism. Keep it. If a future dep upgrade breaks the build, migrate styles to vanilla-extract at that point (it has the same static extraction model) — don't fight Linaria compatibility, just swap the mechanism and keep the `@chamfer-css/studio-tokens` make function calls.

**Implication for phases:** Before Phase 2 (sidebar) and Phase 3 (previews), confirm `@chamfer-css/studio-tokens` is built and linked. New components must use its make functions, not inline styles or arbitrary CSS classes.
...
- `CHAMFER_CSS_STUDIO_CONFIG_PATH`, `CHAMFER_CSS_STUDIO_VERSION_DIR`, `CHAMFER_CSS_STUDIO_IS_LOCAL`, `CHAMFER_CSS_STUDIO_PORT` env var reads
- `getLocalConfigVars()` and `getIsLocalConfig()` in `util.getLocalConfig.ts`
- All `process.env.CHAMFER_CSS_STUDIO_*` reads across the codebase
...
- [x] Delete `getLocalConfigVars`, `getIsLocalConfig`, and all `process.env.CHAMFER_CSS_STUDIO_*` reads
...
The studio package has `.chamfer/tokens.json` as its own dev fixture. `findChamferTokensFile` already walks up from `process.cwd()`, so running `yarn dev` from `packages/chamfer-studio` finds it naturally — no extra scripting needed after Phase 0's `getLoadContext` lands.

Before Phase 0 lands, the vite config or a `.env.local` can inject the env vars as a stop-gap:

```
CHAMFER_CSS_STUDIO_IS_LOCAL=true
CHAMFER_CSS_STUDIO_CONFIG_PATH=/abs/path/to/packages/chamfer-studio/.chamfer/tokens.json
CHAMFER_CSS_STUDIO_VERSION_DIR=/abs/path/to/packages/chamfer-studio/.chamfer/versions
CHAMFER_CSS_STUDIO_PORT=5700
```
...
- [x] Create `packages/chamfer-studio/.chamfer/tokens.json` — committed dev fixture with real token data
- [x] Add `packages/chamfer-studio/.chamfer/versions/` to `.gitignore`
...
`Config.context.tsx` maintains 6 independent `useImmer` slices, each with its own state type that differs from `ChamferTokens`. Adding a token type requires: a new state type, a new `useConfigState*` hook, a new `get*FromState` converter, and a new context entry.
...
```ts
type StudioState = {
  runtime: ChamferTokens["runtime"];
  color: { vibe?: ChamferColorVibe; hue: Record<string, HueEntry>; hex: Record<string, HexEntry> };
...
type StudioContext = {
  ...
  getTokens: () => ChamferTokens; // serializes back to tokens.json shape
...
- [x] Write `initStudioState(tokens: ChamferTokens): StudioState`
- [x] Write `getTokensFromState(state: StudioState): ChamferTokens`
...
chamfer studio    → Express serving the built app — end user, filesystem
```
...
The color system already generates `oklch()` values. `ColorAccessibilityChecker` and `getAccessibleTextColor` in `chamfer-core` have been updated to accept oklch strings (via chroma-js conversion) so the style guide no longer crashes on vibe-based colors.
...
- [x] Add `makeLightDark(light: ColorToken, dark: ColorToken): string` to chamfer-core as a builtin template (auto-exported via `makeUtils.ts`)
...
**Dependency on Phase 6:** ✅ resolved — `makeLightDark` is now a builtin template in chamfer-core.
...
`semanticTemplate.cssProperties()` calls a pure `computeWcagContrast(oklch1: string, oklch2: string): number` helper. Conversion path: parse oklch string → oklch → oklab → linear sRGB → D65-adapted XYZ → relative luminance (WCAG 2.1). Does not throw — logs structured warnings via `console.warn` (the `Chamfer` class caller can suppress these in tests):

```
⚠ [chamfer] semantic.interactive-text on semantic.interactive: 3.8:1 (fails AA 4.5:1)
```

Color tokens are available as oklch strings from the `colorTemplate.tokens(config)` manifest built in the same `Chamfer.build()` pass.
...
- [x] Create `Template.make-semantic-color.ts` in chamfer-core — `tokens()`, `cssProperties()` with WCAG logging, `util()` for `makeSemanticColor`
- [x] Add `computeWcagContrast` helper (pure, no deps) — oklch string → relative luminance → contrast ratio
- [x] Add `semanticTemplate` to `builtinTemplates`, `builtinVarMap`, and `importLines` in `Chamfer.ts`
...
- [ ] Rename the project away from "Chamfer" — keep the `-css` suffix, new word TBD (to be workshopped). This is a real rename, not just a label swap: it touches the npm scope (`@chamfer-css/*`), the CLI binary name (`chamfer studio`, `chamfer build`, etc.), every package directory (`chamfer-core`, `chamfer-cli`, `chamfer-studio`, `chamfer-studio-tokens`), the `.chamfer/` project-folder convention baked into how the tool discovers config, and user-facing UI text ("Chamfer CSS" header, `CHAMFER TOKENS` studio branding). Scope out a migration plan before starting — likely its own phase.
